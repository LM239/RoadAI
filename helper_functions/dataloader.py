...
from helper_functions.schemas import Machine, Trip, Position
import numpy as np
from helper_functions.utils import load_gps_data
...
class TripsLoader:
    ...
    def __init__(self, datestring: str, gps_data_dir: str = "data/GPSData", trips_folder: str ="trips") -> None:
        ...
        def get_trips(id: str):
            ...
            return (
                {"route": grouped_df.get_group(id).drop("TripLogId", axis=1).values}
                if id in unique
                else None
            )
        ...
        for machine_id in info_df["DumperMachineId"].unique():
            ...
            machine_type, machine_name = (
                machine_df.at[0, "MachineType"],
                machine_df.at[0, "DumperMachineName"],
            )
            machine_df = machine_df.drop(["MachineType", "DumperMachineName"], axis=1)
    ...
    def sorted_machines(
        self,
        sort_by: Literal["trip_count", "trip_length", "total_quantity"],
        ...
        key = (
            sort_by_count
            if sort_by == "trip_count"
            else (sort_by_length if sort_by == "trip_length" else sort_by_quantity)
        )

        return sorted(self._machines.values(), key=key, reverse=not descending_order)

    def filter_machine_type(
        self, m_type: Literal["Dumper", "Truck", "Dumpbil"]
    ) -> dict[int, Machine]:
