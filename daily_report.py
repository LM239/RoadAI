...
class Stats(BaseModel):
    ...
    # Inner product of consecutive normalized vectors
...
class DailyReport:
    ...
    def __init__(self, day: str) -> None:
        ...
        self.interactive_map = interactive_map.InteractiveMap(self.trips)
    ...
    def aggregated_idle_timeline(self):
        ...
        first_timestamp = self.idle_machines.list_of_idle_machines[
            0].trips[0].positions[0].timestamp
        # First machines last timestamp
        last_timestamp = self.idle_machines.list_of_idle_machines[
                ...
                for m in self.idle_machines.list_of_idle_machines:
                    for it in m.list_of_idle_times:
                        if it.times[0] < time < it.times[-1]:
                            ...
                            list_of_positions.append(it.points[0])
                            # Check if we are waiting for load or dump
                            # Highest possible value
                            smallest_time_above = m.trips[-1].positions[-1].timestamp
                            waiting_for_load = True
