...
class automated_load_dump_for_machine():
    ...
    def get_data(self):
        ...
        for i in range(1,len(self.stats.all_positions)-1):
            ...
            seconds_gone_i_minus_1 = (current_pos.timestamp-prev_pos.timestamp).total_seconds()
            time_since_last_activity += seconds_gone_i_minus_1

            seconds_gone_i = (next_pos.timestamp-current_pos.timestamp).total_seconds()
    ...
    def get_df_with_ml_data(self):
        ...
        for idx in range(1,len(latitude)-1):
            try:
                total_seconds = (self.stats.day_times[idx] - self.stats.day_times[idx-1]).total_seconds()
...
class LoadDumpLightGBM:
    ...
    def load_data(self):
        ...
        for day in tqdm(days[self.starting_from:self.starting_from+self.nb_days]):
            ...
            df_training_all.to_csv(f"{self.work_dir}/{self.training_data_name}.csv", sep=',', index=False)
            df_testing_all.to_csv(f"{self.work_dir}/{self.test_data_name}.csv", sep=',', index=False)
    ...
    def results(self):
        ...
        df_preds = pd.read_csv(
            ...
            sep=",",
            usecols=["output_labels", "proba_Driving", "proba_Dump", "proba_Load", "predicted_class"],
        )
